...
if not ATHLETE_ID:
    raise EnvironmentError("❌ Missing ATHLETE_ID. Check your .env file or GitHub Secrets.")
...
def upload_workout(workout):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Failed to upload workout for {workout['date']}: {response.status_code} - {error_message}")
