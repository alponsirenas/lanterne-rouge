...
class BackgroundRefreshScheduler:
    ...
    async def start(self):
        ...
        logger.info(f"Background refresh scheduler started (interval: {self.interval_minutes} minutes)")
        ...
        try:
            ...
            for conn in connections:
                try:
                    ...
                    error_count += 1
                    logger.error(
                        f"Failed to refresh {conn.connection_type} for user {conn.user_id}: {str(e)}"
                    )
