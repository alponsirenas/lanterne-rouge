...
class DeliveryAgent:
    ...
    def __init__(self, archive_dir: str = "docs_src/tdf-simulation/tdf-2025-hallucinations"):
...
"""
    ...
    def _save_to_archive(self, content: str, title: str, analysis: AnalysisResult, format: str) -> Path:
        """Save narrative to archive following the stage{number}.md pattern"""

        # Create filename following the docs_src/tdf-simulation/tdf-2025-hallucinations pattern
        stage_num = analysis.stage_data.stage_number
        
        if format == 'markdown':
            filename = f"stage{stage_num}.md"
        else:
            # For other formats, use the original pattern
