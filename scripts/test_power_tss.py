...
def test_power_tss_calculation():
    ...
    run(["python", "scripts/update_athlete_ftp.py", "tdf-sim-2025", "128"])
    ...
    if results:
        ...
        print("=" * 100)
        print(f"{'Date':<12} | {'Type':<12} | {'Avg W':<6} | {'NP':<6} | {'Duration':<10} | {'Power TSS':>9} | {'Strava':>8} | {'ICU':>8} | {'Activity Name':<30}")
        print("-" * 100)
        ...
        for r in sorted(results, key=lambda x: x["activity_date"], reverse=True):
            ...
            print(f"{date:<12} | {r['activity_type']:<12} | {r['avg_watts']:<6} | {r['normalized_watts'] or '?':<6} | {duration_mins:<10} | {r['calculated_power_tss']:>9} | {r['strava_score']:>8} | {r['icu_score']:>8} | {r['activity_name'][:30]}")
    else:
