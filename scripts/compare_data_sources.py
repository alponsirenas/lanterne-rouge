...
def process_activities(activities, days=90):
    ...
    if earliest_date and latest_date:
        print(f"Activities date range: {earliest_date.strftime('%Y-%m-%d')} to {latest_date.strftime('%Y-%m-%d')}")
    else:
        ...
        if csv_tss and strava_tss:
            ...
            for i in range(10):
                ...
                if strava_value != 0 or csv_value != 0:
                    print(f"  {day_key}: Strava={strava_value}, CSV={csv_value}, Diff={strava_value - csv_value}")
